...
/* ==========================================================================
   ...
   ---------------------------------------------------------------------------
   与旧的 assets/js/music.js 的 shell() / listMarkup() 逐块对应：
   暗窗头（现在放哪一首、进页面自动播的是哪一首）、进度、播放控制、音量、
   ...
   空曲库那一句 `<p class="mp-empty">` 故意摆在 `<ul>` **外面**：旧站是把它塞在
   ul 里的，浏览器解析那段 innerHTML 时会把它挤到 ul 后面（p 不能直接住在 ul 里），
   真正渲染出来的就是这个样子——这里照那个结果写，省掉一次水合错位。
   ========================================================================== */
