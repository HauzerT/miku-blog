/* ==========================================================================
   ...
   ---------------------------------------------------------------------------
   与旧的 assets/js/kumura.js 一一对应：扫码登录 → 账号信息 → 我创建的歌单
   → 每日推荐 → 红心歌单，以及这一页自己的播放条。

   形状上和 useRoll.ts 是同一路数——「页面把容器交进来，行为在里面干活」，
   而不是把数据摊成响应式状态让 Vue 重画。原因很实在：这一页的标记要一字不差
   地等于 tools/build.mjs 的 buildKumura()，包括那些空 span、注释里解释过的
   列位、data-* 钩子；Vue 重画一遍只会把这些细节磨掉。所以 pages/kumura.vue
   只吐静态 HTML，一切运行时的事都在这里。
