...
`login.html` 是门口：**访客直接进，站长要口令**。它是自愿走的门，不拦人——
`index.html` 照旧直接打得开，这一页只是把「以什么身份进来」明说一次。
（真要把整站锁起来是另一件事：要么在服务端拦，要么让首页先跳门厅，现在都没做。）

- **访客进入**（粉键）：一个普通链接，直跳首页。没有 JS、没跑服务、`file://` 打开都照样进，
  而且永远不问口令，也不会往浏览器里写任何东西。
- **站长登录**（黑键）：展开一行口令，交给本机的上传服务核对（`POST /api/auth`）。
  对了就把它记进 `localStorage` 的 `cv01-key` —— 与站长工具箱（`studio.js`）用的是
  **同一把钥匙**，所以进门之后右下角那颗钥匙球不会再问第二次。地址栏写
  `login.html#owner` 可以直接停在口令那一行，站长可以把它存成书签。
...
| `cyan` 青窗 | 青底、白块音符、黑字。日间夜间长得一样——它是一块青色的实体，不属于任何主题 |
...
青窗里为什么没有粉：青 `#39C5BB` 的相对亮度是 0.44，**正好落在粉的天敌位置上**——
粉 `#FF3399` 落在它上面只有 **1.60:1**，近白 `#E6EBEF` 只有 **2.13:1**，黑是 **8.83:1**。
所以青窗里只留得下黑：音符是白块（黑字 15.6:1），网格、黑键、播放头全是黑。
"此刻"在窗里也只能是那条黑线。这不是取舍，是算出来的。
