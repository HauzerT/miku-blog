...
**更好的是让浏览器替你记。** 门厅不再拦着浏览器的密码管理器（口令栏是标准的
`autocomplete="current-password"`，登录成功后还会把凭证交给浏览器的密码库——
Chromium 系走 PasswordCredential，Firefox 靠属性自己提示）。第一次登录后在弹出的
「要保存口令吗」里选保存，之后的进门就是：站长登录 → 口令栏已经自己填好
（或用 Windows Hello 解一下锁）→ 进入。凭证在浏览器的密码库里：加密落盘、
随浏览器账号同步、由系统解锁——比躺在 localStorage 里体面，比剪贴板干净。

| 什么 | 活多久 | 管什么 |
|---|---|---|
| 门厅那枚 cookie `cv01-enter` | 30 天 | 页面进不进得去 |
| 口令本身（浏览器 `localStorage` 的 `cv01-key`） | **没有到期时间**——站长工具箱、右键菜单、编辑页认的都是它 | 站长操作 |
| 浏览器密码库里那份凭证 | 跟着浏览器走 | 只是「填进门厅」更省事，与上面两份互不替代 |
