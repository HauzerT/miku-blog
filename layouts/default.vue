...
/* ==========================================================================
   ...
   ---------------------------------------------------------------------------
   外壳按这个顺序吐出来：跳过链接、每日一句、命令栏、轨道栏、正文、页脚、悬浮球。
   这里不拼字符串，而是组件——但吐出来的 class 名与层级是对着 `assets/css/` 写的，
   所以那份样式一个字节都不用改。

   「当前在哪一页」不由页面告诉外壳，而是从路由算出来（`composables/usePageRoutes.ts`
   把 `/sections/<id>`、`/posts/<slug>` 这些地址归成板块 id 与 slug）：页面 setup 跑在
   外壳模板之前，靠 props 传会慢一拍。
   ========================================================================== */
